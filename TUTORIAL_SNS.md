...
FocusFlowが目指すのは、一般的なSNSとは一線を画す、**「静かなる交流」**の場です。いいねの数やフォロワー数を競うのではなく、互いの集中を尊重し、静かに応援し合うことで生まれる穏やかな連帯感を大切にします。
...
これを終えると、ユーザーは他のユーザーをフォローし、そのユーザーの集中セッションを自分のダッシュボードで確認できるようになります。
...
```html
<!-- dashboard.html の自分の集中履歴セクションの下に追加 -->
<section class="card">
...
*   **応援 (Cheer)機能**: アクティビティフィードに、テキスト入力不要のシンプルな「応援」ボタンを設置。クリックすると、相手の集中ゲージにポジティブなエフェクトを与えるなど、集中を妨げずに応援の気持ちを伝えられます。
*   **集中ゲージとの連携**: 将来的に集中ゲージが実装されたら、アクティビティフィードに各ユーザーのリアルタイムの集中度（例: 色の変化）を表示し、より一体感のある体験を創出します。
*   **集中ルーム機能**: 「TOEIC学習部屋」「もくもく開発部屋」など、共通の目的を持つユーザーが集まり、互いの集中状態を可視化しながら作業できるバーチャル空間を実装します。
*   **AIコーチングとの連携**: 蓄積されたデータから「あなたとAさんは週末の朝に最も集中する傾向があります」といった分析を提示したり、互いに良い影響を与え合っている関係性を可視化したりします。
