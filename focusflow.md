...
* **目的**: SNS依存を「フォーカス依存」に転換させ、ユーザーが学習や作業にフォーカスする習慣を身につけられるよう支援する。個人の集中力を可視化・共有し、相互に応援し合う健全なコミュニティを形成する。
* **ターゲットユーザー**: 学生、資格取得を目指す社会人、フリーランス、クリエイターなど、フォーカスして作業に取り組みたいすべての人。特にSNS利用による集中力低下を感じている層。
* **主な価値提案**:
    * **集中力の可視化とモチベーション向上**: 自分の集中度をリアルタイムで確認し、ゲーム感覚でフォーカスを継続できる。
...
* **プロファイル**:
    * **ユーザー名**: 表示名。
    * **ステータス**: フォーカス中、休憩中、オフライン、フロー状態など。
    * **目標**: 現在取り組んでいる大きな目標（例: 「TOEIC 900点」「PythonでWebアプリ開発」など）。
    * **得意分野/興味**: 学習分野やスキルセット（例: 「数学」「プログラミング」「デザイン」など）。
    * **フォーカス履歴**: 日/週/月ごとのフォーカス時間、達成タスク数、フロー状態到達回数などを表示。
    * **設定**: プライバシー設定（フォーカスゲージの公開範囲、他のSNS連携、プロフィール共有設定など）。
...
* **フォーカスゲージ (Focus Gauge)**:
    ...
    * **推定ロジック**:
        ...
        * **AIモデル**: 上記データに基づき、集中度を推定する機械学習モデル（例: ロジスティック回帰、SVM、ニューラルネットワークなど）。モデルの学習には、ユーザーが「フォーカスしていた」と自己申告した時のデータセットや、特定のアプリ（例: VS Codeは集中度高、TikTokは集中度低）との関連付けが利用可能。
...
*   **ブラウザ版での簡易実装ロジック（非AI）**:
    ...
    *   **ゲージの変動ルール**:
        ...
        *   **タブ非表示（短時間）**: ユーザーが他のタブに切り替えるなどしてページが短時間（例: 2分以内）非表示になった場合、それは「意図的な離脱」とみなし、ゲージにペナルティを与え、大幅に減少させる。
...
* **フロー状態検知**:
    * **条件**: AIが推定するフォーカスゲージが一定の閾値（例: 80%以上）を一定時間（例: 5分以上）維持した場合、「フロー状態」と判断し、特殊な視覚効果（画面の縁の光）や通知でユーザーにフィードバック。
    * **共有**: 公開モードの場合、プロフィールに特殊なエフェクトが付与され、他のユーザーに「〇〇さんがフロー状態に入りました！」とアクティビティフィードに通知される（通知は最小限）。
* **セッション終了**:
...
* **ルーム作成**:
    ...
    * **公開/非公開**: パスワード設定も可能。
* **ルーム内表示**:
    * 各参加者のアイコンと**匿名化されたフォーカスゲージ**（具体的な数値ではなく、集中度の色や状態表示のみ）。
...
* **活動フィード（限定的）**:
    * **通知**: フォローしているユーザーがフォーカスセッションを開始/終了した、フロー状態に入ったなどの通知（最小限）。
    * **応援 (Cheer)**:
...
* **フォーカスレポート**:
    ...
    * **AIによる分析とフィードバック**: 「〇曜日の〇時台に集中力が高まりやすい傾向があります」「〇〇アプリの使用後に集中力が低下しています」など、AIが個人のフォーカス傾向を分析し、パーソナライズされた改善アドバイスを提供。
    * **目標達成支援**: 設定した目標に対する進捗状況を可視化。目標達成に向けた次のステップや学習リソースを推薦。
* **集中力改善チャレンジ**: AIがユーザーのデータに基づき、個別の「フォーカスチャレンジ」（例: 「〇分間SNSを開かずにフォーカスする」）を提案。達成でデジタルバッジ付与。

#### 2.6. その他の機能

* **デジタルバッジ/トロフィー**: フォーカス時間達成、フロー状態到達回数、連続フォーカス日数などに応じて付与されるデジタルバッジ。
* **ホワイトリスト/ブラックリストアプリ設定**: ユーザーが自分で「フォーカス中に許可するアプリ」と「フォーカスを妨げるアプリ」を設定。AIの集中度推定ロジックに反映。
...
* **UX/UIデザイン**: フォーカスを妨げない、シンプルで直感的なUI。色使いやアニメーションも集中力に影響を与えるため慎重に。
